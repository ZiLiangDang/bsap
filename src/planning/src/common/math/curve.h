...
namespace Planning
{
    ...
    public:
        ...
        static double NormalizeAngle(const double &angle); // 把角度规范化到[-pi,pi]范围内
        ...
        static int find_match_point(const LocalPath &path, const PoseStamped &target_point);                               // 在路径上查找匹配点
        static int find_match_point(const Referline &path, const double &rs);                                              // 通过rs寻找匹配点下标
        // 找到投影点
        ...
        static void find_projection_point(const LocalPath &path, const PoseStamped &target_point, // 输入：路径和目标点
                                          double &rs, double &rx, double &ry,
                                          double &rtheta, double &rkappa, double &rdkappa); // 输出：投影点在参考线下的参数
        // 计算投影点参数
