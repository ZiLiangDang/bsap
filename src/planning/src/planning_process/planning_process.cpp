...
namespace Planning
{
    ...
    bool PlanningProcess::process() // 总流程
    {
        ...
        if (!planning_init())
        {
            RCLCPP_ERROR(this->get_logger(), "Planning process initialization failed");
            return false;
        }
        //进入规划主流程
    ...
    bool PlanningProcess::planning_init()//流程初始化
    {
        ...
        {
            RCLCPP_ERROR(this->get_logger(), "Failed to connect to map server");
            return false;
        }
        ...
        {
            RCLCPP_ERROR(this->get_logger(), "Failed to connect to global path server");
            return false;
        }
    ...
    template <typename T>
    bool PlanningProcess::connect_server(const T &client)//连接服务
    {
        //判断客户端类型
        ...
        while (!client->wait_for_service(1s))
        {
            if(!rclcpp::ok())
            {
                RCLCPP_ERROR(this->get_logger(),"Interrupted while waiting for the %s service.", server_name.c_str());
                return false;
            }
