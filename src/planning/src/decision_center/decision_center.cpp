...
namespace Planning
{
    ...
    {
        RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Decision_Center created.");
        // 读取配置文件
                                            ...
                                            const std::vector<std::shared_ptr<VehicleBase>> &obses)
    {
        ...
        {
            const double obs_dis_s = obs->s() - car->s(); // 障碍物与主车的距离
            if (obs_dis_s > referline_end_length ||
                obs_dis_s < -least_length) // 如果障碍物在参考线末端的前面//即使接近地图终点，参考线变短，也要考虑最长的距离，防止撞上
            {
                continue;
            }
            ...
            {
                p.s_ = obs->s() + obs->ds_dt() * obs_dis_s / (car->ds_dt() - obs->ds_dt()); // 虚拟障碍物的距离
                                             ...
                                             const std::vector<std::shared_ptr<VehicleBase>> &obses)
    {
        ...
        st_points_.clear();
        const double ori_dis_time = static_cast<double>(decision_config_->local_speeds().speed_size_ - 50);                      // 开始考虑障碍物的时间
        const double ori_dis = ori_dis_time * decision_config_->main_car().speed_ori_;                                           // 开始考虑障碍物的距离
        const double real_brake_time = (ori_dis_time + static_cast<double>(decision_config_->local_speeds().speed_size_)) / 2.0; // 制动的真实距离，取50和100的中间值也就是75
        STPoint p;
        ...
        for (const auto &obs : obses)
        {
            ...
            if(obs_dis_s > ori_dis || //如果车辆还没有进入考虑距离
            ...
            {
                if(fabs(obs->dl_dt_2path())<min_speed)//如果侧向没有速度
                {
                    continue;
                }
                ...
                if (car_dis_time>obs_dis_time && car_dis_time < t_out + delta_t)//让行
                {
                    ...
                    st_points_.emplace_back(p);
                    RCLCPP_INFO(rclcpp::get_logger("decision_center"),"---------------give way,p:(s= %.2f,t=%.2f,ds_dt = %.2f)",p.s_2path_,p.t_,p.ds_dt_2path_);
                }
                else if(car_dis_time < obs_dis_time && car_dis_time > t_in - delta_t)//抢行
                {
                    ...
                    st_points_.emplace_back(p);
                    RCLCPP_INFO(rclcpp::get_logger("decision_center"),"--------------rush out,p:(s=%.2f,t=%.2f,ds_dt=%.2f)",p.s_2path_,p.t_,p.ds_dt_2path_);
                }
