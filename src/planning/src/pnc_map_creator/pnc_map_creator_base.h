...
namespace Planning
{
    ...
    {
    public:
        virtual PNCMap create_pnc_map() = 0; // 创建地图接口
        inline PNCMap pnc_map() const { return pnc_map_; } // 获取地图接口
        inline MarkerArray pnc_map_markerarray() const { return pnc_map_markerarray_; } // 获取地图markerarray接口
        virtual ~PNCMapCreatorBase(){}
    protected:                               // 这里用保护类型，子类中可以直接继承并访问，但对象中不可访问
        std::unique_ptr<ConfigReader> pnc_map_config_; // 配置
        int map_type_ = 0;                             // 地图类型
        PNCMap pnc_map_;                               // 地图
        MarkerArray pnc_map_markerarray_;              // rviz用的地图

        Point p_mid_, ol_, pr_;      // 左中右三个点
