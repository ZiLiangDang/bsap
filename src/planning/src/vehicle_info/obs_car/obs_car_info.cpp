...
namespace Planning
{
                                                    ...
                                                    const std::shared_ptr<VehicleBase> &car)
    {
        ...
        {
            //直接用参考线投影的值近似替代路径投影
            s_2path_ = s_ - refer_line.refer_line[path_end_index].rs;
            ds_dt_2path_ = ds_dt_;
            l_2path_ = l_ - car->l();
            dl_ds_2path_ = dl_ds_;
            dl_dt_2path_ = dl_dt_;
            ...
            RCLCPP_INFO(rclcpp::get_logger("vehicle"),"obs car cartesian_to_fernet to path : path0_rs = %.2f,path_end_rs=%.2f,s2path = %.2f,ds_dt_2path=%.2f,l2path=%.2f,dl_ds_2path=%.2f,dl_dt_2path=%.2f",
        ...
        RCLCPP_INFO(rclcpp::get_logger("vehicle"),"obs car cartesian_to_frenet to path: s2path=%.2f,ds_dt_2path=%.2f,l2path=%.2f,dl_ds_2path=%.2f,dl_dt_2path_=%.2f",
